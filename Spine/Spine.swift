...
open class Spine {
    ...
    open func save<T: Resource>(_ resource: T, path: String?) -> Future<T, SpineError> {
		let promise = Promise<T, SpineError>()
        let operation = SaveOperation(resource: resource, path: path, spine: self)
    ...
    open func delete<T: Resource>(_ resource: T) -> Future<Void, SpineError> {
        return delete(resource, path: nil)
    }
	...
	///
	/// - returns: A future
    open func delete<T: Resource>(_ resource: T, path: String?) -> Future<Void, SpineError> {
		let promise = Promise<Void, SpineError>()
        let operation = DeleteOperation(resource: resource, path: path, spine: self)
